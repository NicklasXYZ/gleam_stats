...
-opaque queue(EAG) :: {queue, list(EAG), list(EAG)}.

-spec new() -> queue(any()).
new() ->
    {queue, [], []}.

-spec from_list(list(EAN)) -> queue(EAN).
from_list(List) ->
    {queue, [], List}.

-spec to_list(queue(EAQ)) -> list(EAQ).
to_list(Queue) ->
...
-spec pop_front(queue(EBI)) -> {ok, {EBI, queue(EBI)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(EBN)) -> queue(EBN).
reverse(Queue) ->
...
-spec is_logically_equal(queue(EBV), queue(EBV), fun((EBV, EBV) -> boolean())) -> boolean().
...
-spec is_equal(queue(EBY), queue(EBY)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
